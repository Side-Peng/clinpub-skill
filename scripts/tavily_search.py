...
Environment Variables:
  TAVILY_API_KEY    Your Tavily API key (get one at https://tavily.com)
    ...
    if not api_key:
        ...
        print("❌ TAVILY_API_KEY 未设置。Tavily 搜索不可用。", file=sys.stderr)
        print("   请通过以下方式之一设置：", file=sys.stderr)
        print(f"   1. 临时设置: export TAVILY_API_KEY=tvly-your_key_here", file=sys.stderr)
        print(f"   2. 添加到 .env 文件: echo 'TAVILY_API_KEY=tvly-xxx' >> .env", file=sys.stderr)
        print(f"   3. 传递 --api-key 参数", file=sys.stderr)
        print(f"   申请地址: https://tavily.com", file=sys.stderr)
        print("=" * 60, file=sys.stderr)
